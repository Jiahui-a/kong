...
|----|------|
| 第 1 列 | 可选序号 |
| **第 2 列** | **料号**（可为空；为空则为空，不继承上一非空行） |
| **第 3 列** | **型号**（可为空；为空则为空，不继承上一非空行） |
...
**料号继承**：Excel 中常见合并单元格写法，后续行只填位号。工具会先**展开合并单元格**再读取；若展开后单元格仍为空，则料号为空（不向上查找上一非空行）。**型号不向上继承**：单元格为空则型号为空，仅当当前单元格（含合并单元格）有值时才使用。某个项目名下的位号为空时跳过该项目内容；仅当**所有项目列均为空**时才跳过该行。
...
上例若料号列 B2:B3 为合并单元格，第 2 行会读到料号 `C12345-001`；型号列未合并则为空（不继承），ProjectA 位号为 `U2`。若型号列也是合并单元格，则第 2 行会读到合并区内的型号。若料号未合并且单元格为空，则该行料号保持为空。
